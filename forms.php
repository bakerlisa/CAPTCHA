    ...
    function form_script_styles(){ ?>
		<script>
			...
			function checkLabels(formNum){
				// session variables - recap
				var recap = sessionStorage.getItem("recapthca");
				sessionStorage.setItem("missedCap","0");
				var recapTriesBeforeComplete = parseInt(sessionStorage.getItem("missedCap"));
			    ...
			    if(recap !== 'complete' &&recapTriesBeforeComplete > 2){
			    	holdThePhone();
			    	holdPopUp();
				}else if(recap !== 'complete'){
					var message = "Must pass Recaptcha First. Try again without recaptcha validation and forms will disappear for a time.";
				    var errorMessage = document.querySelector("#gform_"+formNum + " .form-message");
				    	errorMessage.inerHTML = message;
				    	errorMessage.classList.add('active');
				    	errorMessage.classList.add('recap');

			    	setTimeout(function(){
			    		errorMessage.classList.remove('active');
			    		errorMessage.classList.remove('recap');
			    	},8000);

				    errorMessage.classList.add('error');
				    errorMessage.innerHTML = message;
				    recapTriesBeforeComplete++;
				    sessionStorage.setItem("missedCap",recapTriesBeforeComplete);

				    if(recapTriesBeforeComplete > 1){
				    	holdThePhone();
				    	holdPopUp();	
				    }
			    }else if(alerted > 0){
			...
			function scrollLink(formNum){
				...
				var element = document.getElementById(scro);
				
				var divOffset = offset(element);
				var headerOffset = 200;
				var divGoTo = parseInt(divOffset) - parseInt(headerOffset);
			    window.scrollTo({
			         top: divGoTo,
			         behavior: "smooth"
			    });
